...
build=${WHYFI_BUILD_NUMBER:?WHYFI_BUILD_NUMBER required}
: "${SPARKLE_PRIVATE_KEY:?SPARKLE_PRIVATE_KEY required}"
export GH_TOKEN=${WHYFI_UPDATES_TOKEN:?WHYFI_UPDATES_TOKEN required (write access to the updates repo)}
source ./updates.env

artifact="WhyFi-$version.zip"
[[ -f $artifact ]] || { echo "✗ missing $artifact; prepare did not run" >&2; exit 1; }

# Key via stdin: never on disk, never in argv.
signature=$(printf '%s' "$SPARKLE_PRIVATE_KEY" | /private/tmp/whyfi-build/artifacts/sparkle/Sparkle/bin/sign_update -f - -p "$artifact")
...
public_key=$(/usr/libexec/PlistBuddy -c 'Print :SUPublicEDKey' dist/WhyFi.app/Contents/Info.plist)
...
gh release upload "$tag" -R "$WHYFI_UPDATES_REPO" --clobber "$artifact"
url="https://github.com/$WHYFI_UPDATES_REPO/releases/download/$tag/$artifact"
...
remote="https://github.com/$WHYFI_UPDATES_REPO.git"
...
<channel><title>WhyFi Updates</title><item>
<title>WhyFi $version</title><pubDate>$published</pubDate>
...
rm -rf "$publication"
echo "✓ Published $version → $WHYFI_FEED_URL"
